...
class NetworkBookmark {
    ...
    static func getMyBookmark(userId: String, callback: @escaping (_ modelCafe: [ModelCafe]) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/user/\(userId)/bookmark").responseJSON { (response) in
    ...
    static func setMyBookmark(userId: String, cafeId: String, callback: @escaping (_ description: String) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/user/\(userId)/bookmark", method: .put, parameters: parameters, encoding: JSONEncoding.default).responseJSON { (response) in
