...
class NetworkCafe {
    static func getCafe() {
        let parameter = Location.sharedInstance.currentLocation.toJSON()
        Alamofire.request("\(Config.standardURL)/api/v1/cafe", method: .post, parameters: parameter, encoding: JSONEncoding.default).responseJSON { response in
            switch response.result {
            case .success(let response):
                guard let contents = response as? [[String:Any]] else { return }
                let cafes = Mapper<ModelCafe>().mapArray(JSONArray: contents)
                Cafe.sharedInstance.allCafeList = cafes
...
//        
//        Alamofire.request("\(Config.standardURL)/api/v1/cafe", method: .post, parameters: coordinate.getLocation(), encoding: JSONEncoding.default).responseJSON { (response) in
...
//    }
    ...
    static func getSpecificCafe(cafeId: String, callback: @escaping (_ modelCafe: ModelCafe) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/cafe/\(cafeId)").responseJSON { (response) in
    ...
    static func postCafeReview(review: ModelReview, callback: @escaping (_ modelReviews: [ModelReview], _ rating: Double) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/cafe/\(cafeId)/review", method: .put, parameters: param, encoding: JSONEncoding.default).responseJSON { (response) in
    ...
    static func getCafeReviews(cafeModel: ModelCafe, startIndex: Int=0) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/cafe/\(cafeId)/review").responseJSON { (response) in
            let res = JSON(data: response.data!)
            let reviews = res["reviews"].arrayValue
            let _ = reviews.map {
                let review = $0.dictionaryValue
