...
class NetworkUser {
    ...
    static func register(email: String, password: String, nickname: String, callback: @escaping (_ result: Bool) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/user/register", method: .post, parameters: parameters, encoding: JSONEncoding.default).responseJSON { (response) in
    ...
    static func logIn(email: String, password: String, callback: @escaping (_ result: Bool, _ modelUser: ModelUser) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/user/login", method: .post, parameters: parameters, encoding: JSONEncoding.default).responseJSON { (response) in
    ...
    static func kakaoLogin(email: String, nickname: String, imageURL: String, callback: @escaping (_ result: Bool, _ modelUser: ModelUser) -> Void) {
        ...
        Alamofire.request("\(Config.standardURL)/api/v1/user/login/kakao", method: .post, parameters: parameters, encoding: JSONEncoding.default).responseJSON { (response) in
            var modelUser = ModelUser()
            let res = JSON(data: response.data!)
            let result = res["result"].boolValue
            if let user = res["user"].dictionary {
    ...
    static func getUser(callback: @escaping (_ result: Bool, _ modelUser: ModelUser) -> Void) {
        Alamofire.request("\(Config.standardURL)/api/v1/user/login").responseJSON { (response) in
            var modelUser = ModelUser()
    ...
    static func logout(callback: @escaping (_ description: String) -> Void) {
        Alamofire.request("\(Config.standardURL)/api/v1/user/logout").responseJSON { (response) in
            let res = JSON(data: response.data!)
    ...
    static func getUserImage(userID: String, isKakaoImage: Bool, imageURL: String) -> ImageResource {
        let profileImage = isKakaoImage ? ImageResource(downloadURL: URL(string: imageURL)!, cacheKey: imageURL) : ImageResource(downloadURL: URL(string:  "\(Config.standardURL)/api/v1/user/\(userID)/profileimage/\(imageURL)")!, cacheKey: imageURL)
        return profileImage
    }
    ...
    static func setUserProfileImage(userID: String, image: UIImage, callback: @escaping (_ result: Bool) -> Void) {
        ...
        }, to: "\(Config.standardURL)/api/v1/user/\(userID)/profileimage", method: .put) { (res) in
            switch res {
